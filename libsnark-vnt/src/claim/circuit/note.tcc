...
public:    
    ...
    void generate_r1cs_witness(const Note& notes, const NoteC& notecmtt, uint64_t dist_data, uint64_t subdist_data) { // 为变量生成约束        
        ...
        subcost.fill_with_bits(this->pb, uint64_to_bool_vector(notecmtt.value));
        ...
        dist.fill_with_bits(this->pb, uint64_to_bool_vector(dist_data));
        this->pb.lc_val(dist_packed) = dist.get_field_element_from_bits_by_order(this->pb);

        subdist.fill_with_bits(this->pb, uint64_to_bool_vector(subdist_data));
        this->pb.lc_val(subdist_packed) = subdist.get_field_element_from_bits_by_order(this->pb);

        tmp.fill_with_bits(this->pb, uint64_to_bool_vector(subdist_data * notes.value));

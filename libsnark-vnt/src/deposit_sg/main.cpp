...
boost::optional<r1cs_ppzksnark_proof<ppzksnark_ppT>> generate_deposit_sg_proof(r1cs_ppzksnark_proving_key<ppzksnark_ppT> proving_key,
                                                                    const NoteS& note_s,
                                                                    const Note& note_old,
                                                                    const Note& note,
                                                                    uint256 cmtS,
                    ...
                    const uint256& cmtB                  )
{
    ...
    const r1cs_primary_input<FieldT> input = deposit_sg_gadget<FieldT>::witness_map(
        rt,
        sn_s,
        cmtB_old,
        sn_old,
...
bool test_deposit_sg_gadget_with_instance(
                            uint64_t value,
                            uint64_t value_old,
                            //uint256 sn_old,
                            //uint256 r_old,
                        ...
                        )
{
    ...
    deposit_sg_gadget<FieldT> deposit_sg(pb);
    deposit_sg.generate_r1cs_constraints();// 生成约束
    ...
    auto proof = generate_deposit_sg_proof<default_r1cs_ppzksnark_pp>(keypair.pk, 
                                                            note_s,
                                                            note_old,
                                                            note,
                                                            cmtS,
...
int main () {
    ...
    libff::print_header("#             testing deposit_sg gadget");

    uint64_t value = uint64_t(264); 
    uint64_t value_old = uint64_t(255); 
    uint64_t value_s = uint64_t(9);

    test_deposit_sg_gadget_with_instance<default_r1cs_ppzksnark_pp>(value, value_old, value_s);

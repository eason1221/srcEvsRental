...
r1cs_ppzksnark_proof<ppzksnark_ppT> generate_deposit_sg_proof(r1cs_ppzksnark_proving_key<ppzksnark_ppT> proving_key,
                                                           const Note &note_s,
                                                           const Note &note_old,
                                                           const Note &note,
                                                           uint256 cmtS,
                      ...
                      char *RT)
{
    ...
    for (int i = 0; i < n; i++)
    {
        string subcmt = sss.substr(i * 66, 66);
        commitments[i] = uint256S(subcmt); //分割cmtarray  0x+64个十六进制数 一共66位
    }
...
bool verifyDepositsgproof(char *data, char *RT, char *sn_s, char *cmtb_old, char *snold, char *cmtb)
{
    ...
    alt_bn128_pp::init_public_params();
    r1cs_ppzksnark_keypair<alt_bn128_pp> keypair;
    keypair.vk = deserializevkFromFile("/usr/local/prfKey/depositsgvk.txt");

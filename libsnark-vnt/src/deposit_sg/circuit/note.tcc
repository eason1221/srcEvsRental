...
public:
    ...
    std::shared_ptr<digest_variable<FieldT>> rc; // 256位的随机数r 绑定cost（values）所用的随机数
    ...
    void generate_r1cs_constraints() {
        ...
        rc->generate_r1cs_constraints(); // 随机数的约束
    }

    void generate_r1cs_witness(const Note& note_s, const Note& note_old, const Note& note, const NoteC& notecmtt) { // 为变量生成约束
        ...
        rc->bits.fill_with_bits(this->pb, uint256_to_bool_vector(notecmtt.r));
    }
};